...
String wordsMinute[12] = { "oclock", "five past", "ten past", "fifteen past",
		"twenty past", "twentyfive past", "half past", "twentyfive to",
		"twenty to", "fifteen to", "ten to", "five to" };
String wordsHour[12] = { "twelve", "one", "two", "three", "four", "five", "six",
...
void setup() {
	...
	configManager.setAPName(
			(WORDLAYOUT == 1) ? apName1 :
			(WORDLAYOUT == 2) ? apName2 : apName3);
	configManager.setAPPassword(
			(WORDLAYOUT == 1) ? passwort1 :
			(WORDLAYOUT == 2) ? passwort2 : passwort3);
...
void checkConfig(bool init) {
	...
	bool tempNtpUse = strcmp(config.ntpUse, "yes") == 0
			and configManager.getMode() == 1;
...
void zeigeNachrichtOk() {
	...
	int wortOk[2][2] = { 5 - offsetLayout, 7, 7 - offsetLayout, 7 };
...
void zeigePasswort() {
	...
	int wortPW[2][2] = { 4, 2, 4 + offsetLayout, 3, };
	reiheLedsInListe(wortPW, 2, 3, WORDLAYOUT);
	bearbeiteListe(5);
	FastLED.delay(1000);
	clearLists(3);

	int wortSechsteZeile[10][2] = { 0, 5, 1, 5, 2, 5, 3, 5, 4, 5, 5, 5, 6, 5, 7,
			5 };
